...
export const ShortlistCard: React.FC<ShortlistCardProps> = ({
  ...
  return (
    <TiltCard3D maxTilt={4}>
      <div className="glass-panel-interactive p-5 space-y-4">
        {/* Card Header */}
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-start gap-3.5">
            {/* Rank Badge with gold halo */}
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-50 to-amber-50 border border-rose-200 text-rose-700 font-mono font-black text-sm flex items-center justify-center shrink-0 shadow-sm">
              #{rank}
            </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {candidate.is_fallback && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200">
                    <Cpu className="w-2.5 h-2.5 text-amber-600" /> Fallback
                  </span>
                )}
              </div>
        ...
        <div className="p-3.5 rounded-xl bg-stone-50/90 border border-stone-200 text-xs text-stone-700 leading-relaxed">
          <span className="font-bold text-rose-700 block mb-0.5">Match Rationale:</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 pt-1">
          ...
          <div className="p-3.5 rounded-xl bg-white/70 border border-stone-200 space-y-2.5">
                ...
                <div className="space-y-1">
                  {candidate.matched_requirements.slice(0, 2).map((m, idx) => (
                    <p key={idx} className="text-xs text-stone-700 truncate">
                      • <strong className="text-stone-900">{m.requirement}</strong>: <span className="text-stone-500 italic text-[11px]">"{m.evidence.substring(0, 75)}..."</span>
                    </p>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-1">
                  {candidate.gaps.slice(0, 2).map((g, idx) => (
                    <p key={idx} className="text-xs text-stone-600 truncate">
                      • <span className="text-stone-800 font-semibold">{g.requirement}</span> ({g.reason})
                    </p>
                  ))}
                </div>
              </div>
          ...
          <div className="p-3.5 rounded-xl bg-white/70 border border-stone-200 flex flex-col justify-center">
